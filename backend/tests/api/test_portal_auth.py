...
def test_portal_me_permissions_returns_allowed_modules(client, cable_manager_headers):
    ...
    assert res.status_code == 200
    data = res.json()
    assert set(data["allowed_modules"]) == {"dashboard", "cables", "inquiries", "media", "me", "messages", "resources"}
...
def test_equipment_factory_user_permissions(client, equipment_manager_headers):
    ...
    assert res.status_code == 200
    data = res.json()
    assert set(data["allowed_modules"]) == {"dashboard", "equipment", "inquiries", "media", "me", "messages", "resources"}
