...
async def create_manufacturer(obj_in: ManufacturerCreate, db: AsyncSession = Depends(get_db), user: User = Depends(require_operator("manufacturers"))):
...
async def update_manufacturer(id: str, obj_in: ManufacturerUpdate, db: AsyncSession = Depends(get_db), user: User = Depends(require_operator("manufacturers"))):
    obj = await crud_manufacturer.get(db, id)
    if not obj:
        raise HTTPException(status_code=404, detail={"code": 404, "message": "Manufacturer not found"})
    # Scope check
...
async def delete_manufacturer(id: str, db: AsyncSession = Depends(get_db), user: User = Depends(require_operator("manufacturers"))):
    # Scope check
    if user.role and user.role.scope_type == "manufacturer":
        if id != user.scope_id:
            raise HTTPException(
