# portal-media-management Specification

## Purpose
Defines the manufacturer portal media management capabilities: scoped image uploads to in-scope folders, folder creation within the caller's scope, scoped deletion of own uploads with confirmation (rejecting uploads referenced by entities), a media library page, and a media picker integrated into cable/equipment edit/create forms. All portal media operations route through typed `portalApiClient`/`portalApi` methods and BFF routes that forward the `portal_token` cookie; scope is always derived server-side and never trusted from the client.
## Requirements
