...
export default async function TerminalCategoriesPage() {
  ...
  const rows = tree.flatMap((cat) => {
    const parentRow = (
      <tr key={cat.id} className="border-b border-gray-100 last:border-0">
        ...
        <td className="px-4 py-3">
          <Link
            href={`/admin/connectivity/categories/${encodeURIComponent(cat.id)}`}
    ...
    const childRows = (cat.children ?? []).map((child) => (
      <tr key={child.id} className="border-b border-gray-100 last:border-0 bg-gray-50">
        ...
        <td className="px-4 py-3">
          <Link
            href={`/admin/connectivity/categories/${encodeURIComponent(child.id)}`}
            className="text-accent-foreground hover:underline"
          >
            Edit
          </Link>
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Connectivity Categories</h1>
        <Link
          href="/admin/connectivity/categories/new"
            ...
            {tree.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-gray-400">
                  No connectivity categories found.
                </td>
              </tr>
            )}
          </tbody>
