...
export default async function BrandsPage({ searchParams }: PageProps) {
  ...
  const { items, total } = await adminApi.brands.all(page, PAGE_SIZE);
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));

  // Fetch manufacturers to resolve manufacturer_id -> name for display.
  const mfrRes = await adminApi.manufacturers.all(1, 999);
  const mfrNameById = new Map(mfrRes.items.map((m) => [m.id, m.name]));
