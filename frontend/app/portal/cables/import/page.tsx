...
export default function PortalCableImportPage() {
  ...
  async function handleValidate() {
    ...
    setError(null);
    try {
      const p = await portalApiClient.cables.import.validate(file, format);
      setPreview(p);
      setStage('preview');
    } catch (e) {
  ...
  async function handleCommit() {
    ...
    setError(null);
    try {
      const r = await portalApiClient.cables.import.commit(file, format);
      setResult(r);
      setStage('result');
    } catch (e) {
      setError((e as Error).message);
  ...
  async function handleDownloadCsvTemplate() {
    try {
      const blob = await portalApiClient.cables.import.downloadCsvTemplate();
      triggerBlobDownload(blob, 'portal-cable-import-template.csv');
    } catch (e) {
      setError((e as Error).message);
    }
  }
  ...
  async function handleDownloadJsonExample() {
    try {
      const blob = await portalApiClient.cables.import.downloadJsonExample();
      triggerBlobDownload(blob, 'portal-cable-import-example.json');
    } catch (e) {
      setError((e as Error).message);
    }
  }
