...
export default async function PortalTerminalsPage({ searchParams }: PageProps) {
  ...
  function buildPageHref(p: number): string {
    ...
    return `/portal/connectivity?${params.toString()}`;
  }
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/portal/connectivity/import"
            className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            Import
          </Link>
          <Link
            href="/portal/connectivity/new"
      ...
      {terminals.length === 0 ? (
        <p className="empty-state text-sm text-gray-500">No products in your scope yet.</p>
      ) : (
              ...
              {terminals.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/portal/connectivity/${t.id}`} className="text-accent-foreground hover:underline">
                      Edit
                    </Link>
                  </td>
                </tr>
