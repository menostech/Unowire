...
export default function PortalTerminalsImportPage() {
  ...
  async function handleValidate() {
    ...
    setError(null);
    try {
      const p = await portalApiClient.terminals.import.validate(file, format);
      setPreview(p);
      setStage('preview');
    } catch (e) {
  ...
  async function handleCommit() {
    ...
    setError(null);
    try {
      const r = await portalApiClient.terminals.import.commit(file, format);
      setResult(r);
      setStage('result');
    } catch (e) {
      setError((e as Error).message);
  ...
  async function handleDownloadCsvTemplate() {
    try {
      const blob = await portalApiClient.terminals.import.downloadCsvTemplate();
      triggerBlobDownload(blob, 'portal-products-import-template.csv');
    } catch (e) {
      setError((e as Error).message);
    }
  }
  ...
  async function handleDownloadJsonExample() {
    try {
      const blob = await portalApiClient.terminals.import.downloadJsonExample();
      triggerBlobDownload(blob, 'portal-products-import-example.json');
    } catch (e) {
      setError((e as Error).message);
    }
  }
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            ...
            <div className="space-y-1 text-sm text-gray-600">
              <p><span className="font-medium text-gray-900">{result.created_count}</span> products created</p>
              <p><span className="font-medium text-gray-900">{result.skipped_count}</span> products skipped (already existed)</p>
