...
export default function PortalEquipmentImportPage() {
  ...
  async function handleValidate() {
    ...
    setError(null);
    try {
      const p = await portalApiClient.equipment.import.validate(file, format);
      setPreview(p);
      setStage('preview');
    } catch (e) {
  ...
  async function handleCommit() {
    ...
    setError(null);
    try {
      const r = await portalApiClient.equipment.import.commit(file, format);
      setResult(r);
      setStage('result');
    } catch (e) {
      setError((e as Error).message);
  ...
  async function handleDownloadCsvTemplate() {
    try {
      const blob = await portalApiClient.equipment.import.downloadCsvTemplate();
      triggerBlobDownload(blob, 'portal-equipment-import-template.csv');
    } catch (e) {
      setError((e as Error).message);
    }
  }
  ...
  async function handleDownloadJsonExample() {
    try {
      const blob = await portalApiClient.equipment.import.downloadJsonExample();
      triggerBlobDownload(blob, 'portal-equipment-import-example.json');
    } catch (e) {
      setError((e as Error).message);
    }
  }
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
            ...
            <div
              ...
              onClick={() => document.getElementById('equipment-import-input')?.click()}
            >
              <input
                id="equipment-import-input"
                type="file"
                accept=".csv,.json"
                className="hidden"
                onChange={(e) => {
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            ...
            <div className="space-y-1 text-sm text-gray-600">
              <p><span className="font-medium text-gray-900">{result.created_count}</span> equipment created</p>
              <p><span className="font-medium text-gray-900">{result.skipped_count}</span> equipment skipped (already existed)</p>
