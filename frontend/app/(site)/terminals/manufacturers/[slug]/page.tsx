...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: manufacturer.description?.slice(0, 160) ?? `Learn more about ${manufacturer.name} and their terminals and connectors.`,
  };
}
...
function buildBreadcrumbJsonLd(name: string) {
  return {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Terminals', item: '/terminals' },
      { '@type': 'ListItem', position: 3, name: 'Manufacturers', item: '/terminals#manufacturers' },
      ...
      <div className="space-y-6">
            ...
            <div className="pt-2">
              {memberToken ? (
                <InquiryFormModal
                  ...
                  defaultSubject={`Inquiry about ${manufacturer.name} terminals`}
                />
              ) : (
                <Link
        ...
        <div>
          ...
          {manufacturerTerminals.length === 0 ? (
            <p className="text-gray-500">No terminals listed yet.</p>
          ) : (
