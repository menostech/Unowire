...
export function ResourceCategoryForm({ initial, topCategories }: ResourceCategoryFormProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const url = initial
        ? `/api/admin/resource-categories/${encodeURIComponent(compositeId)}`
  ...
  async function handleDelete() {
    if (!initial) return;
    if (!window.confirm('Delete this resource category?')) return;
    try {
      const res = await fetch(`/api/admin/resource-categories/${encodeURIComponent(initial.id)}`, {
        method: 'DELETE',
      });
